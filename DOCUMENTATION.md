...
The executables are also ignored because the patched Codex executable is larger than GitHub's normal per-file limit. Distribute them as GitHub Release assets and place them in `bin/` before installation.

## Prerequisites

- Windows PowerShell.
- Codex `0.154.0-alpha.6.2`.
- The matching release executables placed under `bin/`.
...
### 1. Obtain the release executables

Download these two artifacts from the matching GitHub Release and place them in `bin/`:
...
Verify their published SHA-256 hashes before enabling the kit. The validated compatibility executable has this hash:
